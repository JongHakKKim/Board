...
class ReplyServiceImplTest {
    ...
    @Test
    @Transactional
    public  void vafdsdfsa(){

/*        Page<Reply> replyPage =
        replyRepository.findByBoardBno(298L, PageRequest.of(0,10));

        replyPage.getContent().forEach(reply ->  log.info(reply));*/
        PageResponseDTO<ReplyDTO> responseDTO =
        replyService.pageList(new PageRequestDTO() ,298L);

        responseDTO.getDtoList().forEach( replyDTO ->  log.info(replyDTO));
