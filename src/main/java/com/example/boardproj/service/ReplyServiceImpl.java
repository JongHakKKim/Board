...
public class ReplyServiceImpl implements ReplyService{
    ...
    public void register(ReplyDTO replyDTO) {
        ...
        Board board =optionalBoard.orElseThrow(EntityExistsException::new);
        //없을때는 예외처리
    ...
    public ReplyDTO read(Long rno) {
        ...
        Reply reply=
        replyRepository.findById(rno).orElseThrow(EntityExistsException::new);
    ...
    public List<ReplyDTO> list(Long bno) {
        ...
        // list에서 하나순차적으로 0부터 꺼내서 꺼낸아이를 변수 reply라 정하고
        // 이걸가지고 RpplyDTO 타입으로 변환한다음 리턴값으로 ReplyDTO 타입으로
    ...
    public Page<ReplyDTO> listPage(Long bno, Pageable pageable) {
        ...
        // 내부적으로 limit을 사용할 수 있도록 갑ㅅ을 주며 ,
        // 현재 페이지당 보여질 객체수를 결정해 줍니다.
        // 위 2가지만 설정가능하며(오버로딩) , 추가로 정렬조건 또한 줄수 있습니다.
        // 현재는 만들어진 pageable 을 받았으나 컨트롤러에서 만들어서 넣어주던
   ...
   /*     Pageable ex =
    ...
    public ReplyDTO update(ReplyDTO replyDTO) {
        ...
        Reply reply =
        replyRepository.findById(replyDTO.getBno())
                .orElseThrow(EntityExistsException::new);
        //수정할 데이터를 수정한다. // 댓글 내용만 수정할것이다.
